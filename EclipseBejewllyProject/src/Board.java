...
class Board {
    ...
    public void printInstructions() {
        ...
        System.out
                .println("- For each gem greater than 3 in a chain it becomes");
        System.out
                .println("- a multiplier. e.g. a 4 gem chain means the score is multiplied");
        System.out
                .println("- by 2, 5 gem chain means score is multiplied by 3 and so on...");
        System.out.println("- Type HELP at any point to see this again.");
        System.out.println("- Type RELOAD at any point to start over.");
        System.out
                .println("- Type CHECK to see if there are still available moves.");
    ...
    public void promptUser() {
        ...
        String selected_piece = console
                .readLine("Which piece would you like to move? (e.g 1A) ");
        if (selected_piece.contains("*")) {
            selected_piece = selected_piece.replace("*", "");
            int[] piece_array = getCoordinates(selected_piece);
            System.out.println("Piece at " + selected_piece + " is "
                    + this.boardGrid[piece_array[0]][piece_array[1]]);
            promptUser();
        }
        ...
        String direction_input = console
                .readLine("In which direction to swap? (U,D,L,R) ");
        ...
        if (swapping_piece != null) {
            ...
            if (did_swap) {
                ...
                System.out.println("YAY! You've made a chain. " + turnScore
                        + " points.");
            } else {
    ...
    private Boolean checkForKeyWords(String input) {
        ...
        if (input.contains("CHECK") || input.contains("check")) {
            ...
            if (checkArray != null && checkArray.length > 0) {
                ...
                System.out
                        .println("There are no more moves available. Feel free to RELOAD.");
            }
            return true;
        }
        if (input.contains("HINT") || input.contains("hint")) {
            int[] checkArray = areMovesStillAvailable();
            if (checkArray != null && checkArray.length > 0) {
                System.out.println("A sequence can be made if "
                        + (checkArray[1] + 1)
                        + getCharForNumber(checkArray[0] + 1) + " is swapped");
            } else {
                System.out
                        .println("There are no more moves available. Feel free to RELOAD.");
            }
        ...
        for (int i = 0; i < BOARD_SIZE; i++) {
            for (int j = 0; j < BOARD_SIZE; j++) {
                ...
                for (String direction : directions) {
                    int[] neighborPiece = getSwappingPiece(direction,
                            currentLocation);
                    if (neighborPiece != null) {
                        int[] pieceCanSequence = temporarilySwapAndCheckPieces(
                                neighborPiece, currentLocation);
                        if (pieceCanSequence != null
                                && pieceCanSequence.length > 0) {
    ...
    private Boolean isValidEntry(char[] input_array) {
        if (input_array.length > 1) {
            ...
            if (firstPosition.matches("[1-" + BOARD_SIZE + "]")
                    && secondPosition.matches("[a-hA-H]")) {
                return true;
            } else {
                return false;
            }
    ...
    private int[] getCoordinates(String selected_piece) {
        ...
        int y = getNumberForCharacter(Character.toUpperCase(selected_piece
                .charAt(1))) - 1;
